...
namespace GeometryAPI
{
    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        public double GetArea()
        {
            double AreaBase = Math.PI * this.Radius * this.Radius;
            double Height = 2 * Math.PI * this.Radius * ((this.Bottom - this.Top).Magnitude);
            return Height + 2 * AreaBase;
        ...
        {
            return Math.PI * this.Radius * this.Radius * ((this.Bottom - this.Top).Magnitude);
        }
    }
    ...
    public class AdvanceFigureController : FigureController
    {
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "area":
                    {
                        IAreaMeasurable currentAreaMeasurable = this.currentFigure as IAreaMeasurable;
                        if (currentAreaMeasurable != null)
                        {
                            Console.WriteLine("{0:0.00}", currentAreaMeasurable.GetArea());
                        }
                        else
                        {
                ...
                case "volume":
                    {
                        IVolumeMeasurable currentVolumeMeasurable = this.currentFigure as IVolumeMeasurable;
                        if (currentVolumeMeasurable != null)
                        {
                            Console.WriteLine("{0:0.00}", currentVolumeMeasurable.GetVolume());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
                ...
                case "normal":
                    {
                        IFlat currentNormal = this.currentFigure as IFlat;
                        if (currentNormal != null)
                        {
                            Console.WriteLine("{0:0.00}", currentNormal.GetNormal());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
